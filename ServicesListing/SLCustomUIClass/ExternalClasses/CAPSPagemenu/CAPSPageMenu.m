...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(languageChanged) name:@"LANGUAGE" object:nil];
...
- (void)setUpUserInterface
{
    ...
    _controllerScrollView.frame = CGRectMake(0.0, _menuHeight, windowWidth, self.view.frame.size.height);
    ...
    _menuScrollView.frame = CGRectMake(0.0, 0.0, windowWidth, _menuHeight);
...
//        _menuScrollView.contentSize = CGSizeMake((_menuItemWidth + _menuMargin) * (CGFloat)_controllerArray.count + _menuMargin, _menuHeight);
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (controller.title != nil) {
            ...
            [menuItemView setTitleText:SLLocalizedString(menuItemView.strViewTitle)];
        } else {
...
-(void)languageChanged{
    ...
    for (MenuItemView *itemViewTemp in _menuScrollView.subviews) {
        ...
        if ([itemViewTemp isKindOfClass:[MenuItemView class]]) {
            ...
            itemViewTemp.titleLabel.text = SLLocalizedString(itemViewTemp.strViewTitle);
        }
    }
