//
//  SLServicesDataViewController.m
//  ServicesListing
//
...
#import "SLServicesDataViewController.h"

static NSString *cellIdentifier = @"SLServicesTableViewCell";

@interface SLServicesDataViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,serviceDetailProtocol>{
...
@implementation SLServicesDataViewController
...
- (void)getSortString:(NSNotification*)notif {
    ...
    [self makeWebApiCallForMyServiceList];
}
...
- (void)addObserver {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateSearchString:) name:KNOTIFICATION_SEARCH_VIEW_EDITED object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(callSearchApi:) name:@"callSearchApi" object:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([APPDELEGATE isArabic]) {
        ...
        [cell.btnLike setSemanticContentAttribute:UISemanticContentAttributeForceRightToLeft];
        [cell.btnComment setSemanticContentAttribute:UISemanticContentAttributeForceRightToLeft];
        [cell.btnDistance setSemanticContentAttribute:UISemanticContentAttributeForceRightToLeft];
    }
    else {
        ...
        [cell.btnLike setSemanticContentAttribute:UISemanticContentAttributeForceLeftToRight];
        [cell.btnComment setSemanticContentAttribute:UISemanticContentAttributeForceLeftToRight];
        [cell.btnDistance setSemanticContentAttribute:UISemanticContentAttributeForceLeftToRight];
    }
...
- (void)makeWebApiCallForMyServiceList {
    ...
    [[ServiceHelper_AF3 instance] makeWebApiCallWithParameters:dictRequest AndPath:apimy_services WithCompletion:^(BOOL suceeded, NSString *error, id response) {
...
- (void)makeWebApiCallForSearchServiceList:(NSString *)str {
   ...
   // NSDictionary *tempDict = notif.userInfo;
    [dictRequest setValue:[NSString stringWithFormat:@"%ld",(long)self.serviceType] forKey:KserviceType];
    [dictRequest setValue:@"1" forKey:KownserviceSearch];
